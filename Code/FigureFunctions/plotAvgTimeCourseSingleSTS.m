function PlottedMean=plotAvgTimeCourseSingleSTS(adaptDataList,params,conditions,binwidth,trialMarkerFlag,indivFlag,indivSubs,colorOrder,biofeedback,removeBiasFlag,labels,medianFlag,plotHandles, row, col, sub, afterTrialP, earlyValues,lateValues)
...
if nargin<17 || isempty(afterTrialP)
    afterTrialP=5;
end

if nargin<18 || isempty(earlyValues)     
     earlyValues=[];
elseif isnumeric(earlyValues)
    if length(earlyValues)==1
        earlyValues=abs(earlyValues);
    elseif length(earlyValues)>1
        earlyValues=abs(earlyValues(1));
        warning ('More than one value provided, only the first one in the array used');
    end
end

if nargin<19 || isempty(lateValues)   
    lateValues=[];
elseif isnumeric(lateValues) 
    if length(lateValues)==1
        lateValues=abs(lateValues);
    elseif length(lateValues)>1
        lateValues=abs(lateValues(1));
        warning ('More than one value provided, only the first one in the array used');
    end 
end
...
for group=1:Ngroups
    for subject=1:length(adaptDataList{group})
        ...
        elseif removeBiasFlag==2
            adaptData = adaptData.normalizeBias('slow');%CJS to make like OGAWA 2014
...
%                 %% CJS added this 10/12/2018 in order to be able to plot two parameters on one plot
        ...
        for t=1:length(fields(values(group).(params{p}).(cond{c})));
            ...
            for p=1:length(params)
                ...
                for i = 1:length(start)-1                    
                    t1 = start(i+1); %So, I ignore the very first stride added by Marcela 02-22-2019 
                    t2 = stop(i+1);
                    bin = allValues(:,t1:t2);
                ...
                if ~isempty(earlyValues) && ~isempty(lateValues)
                    if strcmp(cond{c},{'TMbase'})
                        
                        afterTrialPad=afterTrialP;                        

                        y=[avg(group).(params{p}).(cond{c}).(['trial' num2str(t)])(1,end-lateValues+1:end), NaN(1,afterTrialPad)]; %Plot only late baseline
                        E=[se(group).(params{p}).(cond{c}).(['trial' num2str(t)])(1,end-lateValues+1:end), NaN(1,afterTrialPad)];
                        condLength=length(y);
                        x=Xstart:Xstart+condLength-1;
                    elseif strcmp(cond{c},{'adaptation'}) || strcmp(cond{c},{'TMpost'})
                        afterTrialPad=1; %adds empty sapce to end of trial/condition (in strides)
                        truncatedPad=20;
                        
                        if lateValues==0
                            y=[avg(group).(params{p}).(cond{c}).(['trial' num2str(t)])(1,1:earlyValues), NaN(1,afterTrialPad)]; %Plot only late baseline
                            E=[se(group).(params{p}).(cond{c}).(['trial' num2str(t)])(1,1:earlyValues),  NaN(1,afterTrialPad)];
                            condLength=length(y);
                            x=Xstart:Xstart+condLength-1;
                        elseif earlyValues==0 || isempty(earlyValues)
                            y=[avg(group).(params{p}).(cond{c}).(['trial' num2str(t)])(1,end-lateValues+1:end),  NaN(1,afterTrialPad)]; %Plot only late baseline
                            E=[se(group).(params{p}).(cond{c}).(['trial' num2str(t)])(1,end-lateValues+1:end), NaN(1,afterTrialPad)];
                            condLength=length(y);
                            x=Xstart:Xstart+condLength-1;
                        else 
                            y=[avg(group).(params{p}).(cond{c}).(['trial' num2str(t)])(1,1:earlyValues), NaN(1,truncatedPad), avg(group).(params{p}).(cond{c}).(['trial' num2str(t)])(1,end-lateValues+1:end),  NaN(1,afterTrialPad)]; %Plot only late baseline
                            E=[se(group).(params{p}).(cond{c}).(['trial' num2str(t)])(1,1:earlyValues),  NaN(1,truncatedPad), se(group).(params{p}).(cond{c}).(['trial' num2str(t)])(1,end-lateValues+1:end), NaN(1,afterTrialPad)];
                            condLength=length(y);
                            x=Xstart:Xstart+condLength-1;
                        end
                    ...
                    if strcmp(cond{c},{'TMbase'})
                        afterTrialPad=30; %adds empty sapce to end of trial/condition (in strides)
                    else
                ...
                if indivFlag %plotting all individual subjects
                    ...
                    if Ngroups==1 && ~(size(params,1)>1) && isempty(biofeedback)  %one group (each condition colored different)
                        if isempty(biofeedback)
                            if size(colorOrder,1)==1 %Added to plot all the conditions in one same color))
                                [Pa, Li{c}]=nanJackKnifeSTS(x,y,E,colorOrder(1,:),colorOrder(1,:)+0.5.*abs(colorOrder(1,:)-1),0.7);
                            else                                
                                [Pa, Li{c}]=nanJackKnifeSTS(x,y,E,colorOrder(c,:),colorOrder(c,:)+0.5.*abs(colorOrder(c,:)-1),0.7);
                            end 
                        else
                            [Pa, Li{c}]=nanJackKnifeSTS(x,y,E,colorOrder(c,:),colorOrder(c,:)+0.5.*abs(colorOrder(c,:)-1),0.7,w);
                        end
                    ...
                    elseif size(params,1)>1 && isempty(biofeedback)%Each parameter colored differently (and shaded differently for different groups)
                        ...
                        color=colorOrder(ind,:);
                        [Pa, Li{ind}]=nanJackKnifeSTS(x,y,E,color,color+0.5.*abs(color-1),0.7);
...
%                        legendStr{ind}=legStr;
                    elseif  isempty(biofeedback) %Each group colored differently
                        ...
                        [Pa, Li{g}]=nanJackKnifeSTS(x,y,E,color,color+0.5.*abs(color-1),0.7);
                    ...
                    elseif ~(size(params,1)>1) && ~isempty(biofeedback)
                        color=colorOrder(g,:)./Cdiv;
                        [Pa, Li{g}]=nanJackKnifeSTS(x,y,E,color,color+0.5.*abs(color-1),0.7,w);
                        %set(Li{g},'Clipping','off')
                        H=get(Li{g},'Parent');
...
%             if find(strcmp(conditions, 'adaptation'))==c && group==nanmax(Ngroups)% CJS 5/7/2018 
