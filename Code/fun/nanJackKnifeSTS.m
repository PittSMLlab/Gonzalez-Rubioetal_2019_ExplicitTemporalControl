function [Pa,Li] = nanJackKnifeSTS(varargin)
...
indxF=find(diff(isnan(y))==1);
indxI=find(diff(isnan(y))==-1)+1;

xnew=x(~isnan(y));

if isempty(indxI) && indxF(end)==xnew(end) && length(indxF)==1
   
    Xcoords = [x(1:indxF) x(indxF:-1:1)];
    Ycoords = [U(1:indxF) L(indxF:-1:1)];
...
elseif length(indxF)>1 && length(indxI)==length(indxF)-1
    
    indxI=[1 indxI];
        
    for i=1:length(indxI)
        
        Xcoords = [x(indxI(i): indxF(i)) x(indxF(i):-1:indxI(i))];
        Ycoords = [U(indxI(i): indxF(i)) L(indxF(i):-1:indxI(i))];
