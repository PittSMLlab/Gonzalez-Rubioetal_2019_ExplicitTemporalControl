function varargout=plotAvg(adaptDataList,params,conditions,binwidth,trialMarkerFlag,indivFlag,indivSubs,colorOrder,biofeedback,removeBiasFlag,labels,filterFlag,plotHandles,alignEnd,alignIni,removeBadStridesF)
...
%removeBadStrides if you dont want to do this put 'n' as input
...
if nargin<14 | alignEnd==0 | isempty(alignEnd)
    alignEnd=[];
end

if nargin<15 | alignIni==0 | isempty(alignIni)
    alignIni=[];
end

if nargin<16 || isempty(removeBadStridesF)
    rBSF=[];
elseif ~isempty(removeBadStridesF) && (strcmp(removeBadStridesF,'n') ||(strcmp(removeBadStridesF,'N')))
    rBSF=1;
else 
    rBSF=[];
end
...
for group=1:Ngroups
    for subject=1:length(adaptDataList{group})
        ...
        if isempty(rBSF) %I added this for the excludeDataParams output to be able to be plotted with this function 
